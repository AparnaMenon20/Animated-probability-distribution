...
Draw samples from a exponential distribution and dynamically
...
def update_frame(frame_number, event_count, values, frequencies):
    ...
    for _ in range(event_count):
        while True:
            index = int(np.random.exponential(8))
    ...
    show_bar_chart(f'Frequencies of {freq_sum:,} Exponentially Distributed Random Values ' +
                   f'(Frame {frame_number + 2})\n  µ = 12.5 and Pr(X≤20) = (1−e−20/12.5) ~= 0.7981',
                   'Value', 'Frequency',
...
exponential_animation = animation.FuncAnimation(
